...
{
    public partial class updStrands : Form
    {
        private int strandID;
        public updStrands(int id, string departmentName, string strandName)
        {
            InitializeComponent();
            strandID = id;
            LoadDepartments();
            cmbDepartments.Text = departmentName;
            txtStrand.Text = strandName;
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(classDatabase.ConnectionString))
            {
                ...
                string checkQuery = "SELECT COUNT(*) FROM strands WHERE departmentID = @departmentID AND strandName = @strandName AND strandID != @strandID";
                ...
                if (count > 0)
                {
                    MessageBox.Show("This strand already exists in the selected department.", "Duplicate Strand", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                string query = "UPDATE strands SET departmentID = @departmentID, strandName = @strandName WHERE strandID = @strandID";
                MySqlCommand cmd = new MySqlCommand(query, conn);
            ...
            MessageBox.Show("Strand updated successfully.", "Success!", MessageBoxButtons.OK, MessageBoxIcon.Information);
            this.Close();
