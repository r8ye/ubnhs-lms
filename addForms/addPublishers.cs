...
namespace ubnhs_lms.addForms
{
    public partial class addPublishers: Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(publisher) || string.IsNullOrEmpty(contactNo) || string.IsNullOrEmpty(email))
            {
            ...
            {
                conn.Open();
                string checkQuery = "SELECT COUNT(*) FROM publishers WHERE PublisherName = @PublisherName";
                MySqlCommand checkCmd = new MySqlCommand(checkQuery, conn);
                checkCmd.Parameters.AddWithValue("@PublisherName", publisher);
                int count = Convert.ToInt32(checkCmd.ExecuteScalar());

                if (count > 0)
                {
                    MessageBox.Show("This publisher already exists.", "Duplicate Found", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
                    string query = "INSERT INTO publishers (PublisherName, ContactNo, Email) VALUES (@PublisherName, @ContactNo, @Email)";
                    ...
                    MessageBox.Show("Publisher saved successfully.", "Success!", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    txtPublisher.Clear();
                    txtContactNo.Clear();
                    txtEmail.Clear();
                    this.Close();
